...
class X86JIT {
  public:
    ...
    void add(jit_code_t *jit,uint8_t count){
      ...
      jit->code_buf[jit->code_size + 7] = static_cast<char>(count & 0xFF); // hex value
      memcpy(jit->code_buf + jit->code_size + 8, 
    ...
    void inc(jit_code_t *jit,uint8_t count){
      ...
      jit->code_buf[jit->code_size+4] = static_cast<char>(count & 0xFF);
      jit->code_size += 5;
